const styles = {
  custom_container: "w-11/12 hidden sm:block",
  heading:
    "text-[27px] text-center md:text-start font-[600] font-Roboto pb-[20px]",
  section: "w-11/12 mx-auto",
  productTitle: "text-[25px] font-[600] font-Roboto text-[#333]",
  productDiscountPrice:
    "font-bold text-[14px] 600px:text-[18px] text-[#333] font-Roboto",
  price:
    "font-[500] 600px:text-[16px] text-[12px] text-[#d55b45] pl-3 mt-[-4px] line-through ",
  shop_name:
    "600px:pt-3 600px:text-[15px] 600px:text-blue-400 600px:pb-3  pt-2 text-[12px] text-blue-400 pb-2",
  active_indicator: "absolute bottom-[-27%] left-0 h-[3px] w-full bg-[crimson]",
  button:
    "w-[150px] bg-black h-[50px] my-3 flex items-center justify-center rounded-xl cursor-pointer",
  cart_button:
    "px-[20px] h-[38px] rounded-[20px] bg-[#f63b60] flex items-center justify-center cursor-pointer",
  cart_button_text: "text-[#fff] text-[16px] font-[600]",
  input: "w-full border p-1 rounded-[5px]",
  activeStatus:
    "w-[10px] h-[10px] rounded-full absolute top-0 right-1 bg-[#40d132]",
  normalFlex: "flex items-center",
  authInput: `appearance-none block w-full px-3 py-2 border border-gray-300 
  rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-400 focus:border-blue-400 sm:text-sm`,
  inputStyle: "w-full bg-transparent  px-1 outline-none text-base",
  containerStyle:
    " items-center flex  border-gray-400 border rounded-md px-3  focus-within:border-2 focus-within:border-gray-700",
};

export default styles;
